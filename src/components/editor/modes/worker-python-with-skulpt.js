...
define('ace/worker/mirror', [], function (require, exports, module) {
    ...
    var SkRun = require('../worker/skulpt').run;
...
define('ace/mode/python_worker', [], function (require, exports, module) {
    ...
    var parse = require('../worker/skulpt').parse;
...
define('ace/worker/skulpt', [], function (require, exports, module) {
    importScripts(
        location.origin + '/skulpt/skulpt.js',
        location.origin + '/skulpt/skulpt-stdlib.js'
        // location.origin + '/skulpt/debugger.js'
    );
    const Sk = window.Sk;
    // finds lines starting with "print"
    const re = new RegExp('\\s*print');
    // finds import statements
    const importre = new RegExp('\\s*import');
    // finds multuline string constants
    const mls = new RegExp("'''");
    // finds defining statements
    const defre = new RegExp('def.*|class.*');
    // test for empty line.
    ...
    const assignment = /^((\s*\(\s*(\s*((\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*)|(\s*\(\s*(\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*,)*\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*\)\s*))\s*,)*\s*((\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*)|(\s*\(\s*(\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*,)*\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*\)\s*))\s*\)\s*)|(\s*\s*(\s*((\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*)|(\s*\(\s*(\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*,)*\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*\)\s*))\s*,)*\s*((\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*)|(\s*\(\s*(\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*,)*\s*((\s*[_a-zA-Z]\w*\s*)|(\s*\(\s*(\s*[_a-zA-Z]\w*\s*,)*\s*[_a-zA-Z]\w*\s*\)\s*))\s*\)\s*))\s*\s*))=/;

    let inputResolver = null;
    let inputRejecter = null;
    const terminal = {
        log (msg) {
            window.sender.emit('terminal.log', msg);
        },
        error (msg) {
            window.sender.emit('terminal.error', msg);
        },
        indent (msg) {
            window.sender.emit('terminal.indent', msg);
        },
        executed (msg) {
            window.sender.emit('terminal.executed', msg);
        },
        input (msg) {
            return new Promise((resolve, reject) => {
                window.sender.emit('terminal.input', msg);
                inputResolver = resolve;
                inputRejecter = reject;
            });
        }
    };
    window.sender.on('terminal.reply', (data) => {
        if (inputResolver) {
            inputResolver(data.data);
            inputResolver = null;
        }
    });
    window.sender.on('terminal.cancel', () => {
        if (inputRejecter) {
            inputRejecter();
            inputRejecter = null;
        }
    ...
    const onError = (err, lines, filename) => {
        if (typeof lines === 'string') {
            lines = lines.split('\n');
        }
        try {
            const errArgs = err.args.v;
            const traceback = err.traceback ? err.traceback[0] : {};
            const errorInfo = {
                type: 'error',
                name: err.tp$name,
                filename,
                text: err.toString(),
                row: traceback.lineno - 1,
                raw: '',
                column: traceback.colno - 1,
                context: errArgs[3]
            };
            errorInfo.message = `  File "${errorInfo.filename}", line ${errorInfo.row + 1}\n` +
            (Array.isArray(errorInfo.context) ? (
                `    ${errorInfo.context[2].replace(/\n$/, '')}\n` +
                `    ${' '.repeat(errorInfo.context[1][1])}^\n`
            ) : `    ${lines[errorInfo.row]}\n`) +
            `${err.toString()}`;
            return errorInfo;
        } catch (err) {
    ...
    const editorRun = (data) => {
        if (data.data) {
            const resPromise = Sk.misceval.asyncToPromise(() =>
                Sk.importMainWithBody('<stdin>', false, data.data, true)
            );
            resPromise.catch(err => {
                terminal.error(onError(err, data.data, '<stdin>'));
            });
        }
    };
    const parse = (filename, code) => {
        if (code) {
            try {
                Sk.parse(filename, code);
            } catch (err) {
                return onError(err, code.split('\n'), filename);
            }
        }
    };

    const terminalRun = (data) => {
        if (data.data) {
            ...
            try {
                ...
                if (!lines || /^\s*$/.test(lines)) {
                    ...
                    Sk.misceval.asyncToPromise(function () {
                        return Sk.importMainWithBody('<stdin>', false, lines.join('\n'), true);
                    }).then(() => {
                        window.sender.emit('terminal.executed', null);
                    }).catch(err => {
                        terminal.error(onError(err, lines, '<stdin>'));
                        console.log(err);
                    });
                }
            } catch (err) {
    ...
    window.sender.on('terminal.run', terminalRun);
    window.sender.on('run', editorRun);

    exports.Sk = Sk;
    exports.run = editorRun;
    exports.parse = parse;
});
