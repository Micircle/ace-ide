...
*/
  ...
  //
  //
  var add_identnode = ($B.parser.add_identnode = function(
    parent,
    insert_at,
    name,
    val
    ...
    if (insert_at === -1) parent.add(new_node);
    else parent.insert(insert_at, new_node);
   ...
   * $YieldFromMarkerNode.
   */
  var $add_yield_from_code = ($B.parser.$add_yield_from_code = function(
    yield_ctx
  ) {
    ...
    */
    ...
    if (yield_ctx.parent.type === "assign") {
      ...
      params.save_result_rank =
        pnode.parent.children.length - pnode.parent.children.indexOf(pnode);
    }
...
*/
  ...
  var $_SyntaxError = ($B.parser.$_SyntaxError = function(
    context,
    msg,
    indent
  ) {
...
*/
  ...
  var $YieldFromMarkerNode = ($B.parser.$YieldFromMarkerNode = function(
    params
  ) {
...
*/
  ...
  var $AbstractExprCtx = ($B.parser.$AbstractExprCtx = function(
    context,
    with_commas
  ) {
    ...
    this.transform = function(node, rank) {
      ...
      switch (left.type) {
        case "expr":
          if (left.tree.length > 1) {
            ...
            left.tree[0].type == "list_or_tuple" ||
      ...
      if (right_items !== null) {
        ...
        if (right_items.length > left_items.length) {
          ...
          throw Error(
            "ValueError : need more than " + right_items.length + " to unpack"
          );
        }
        var new_nodes = [],
          pos = 0;
            ...
            if (
              ...
              return [
                js +
                  ...
                  ".$is_class ? " +
                  js +
                  ".__dict__.$string_dict['" +
                  this.name +
                  "'] = ",
                " : $B.$setattr(" + js + ', "' + this.name + '", '
              ];
            }
          }
        }
  ...
  var $AugmentedAssignCtx = ($B.parser.$AugmentedAssignCtx = function(
    context,
    op
  ) {
    ...
    if (context.type == "expr") {
      ...
      if (assigned.type == "id") {
        ...
        if (noassign[name] === true) {
          ...
          (scope.ntype == "def" || scope.ntype == "generator") &&
    ...
    this.transform = function(node, rank) {
      ...
      var left_is_id =
        this.tree[0].type == "expr" && this.tree[0].tree[0].type == "id";
      ...
      var right_is_int =
        this.tree[1].type == "expr" && this.tree[1].tree[0].type == "int";
        ...
        case "/=":
          if (left_is_id) {
            ...
            switch (scope.ntype) {
              ...
              case "def":
              case "generator":
                if (scope.globals && scope.globals.has(context.tree[0].value)) {
                  prefix = global_ns;
                } else {
                  prefix = "$locals";
                }
      ...
      assign2.tree.push(
        $NodeJS(
          "$B.$getattr(" + context.to_js() + ',"' + func + '")(' + right + ")"
        )
      );
  ...
  var $CallCtx = ($B.parser.$CallCtx = function(context) {
    ...
    if (
      this.func &&
      this.func.type == "attribute" &&
      this.func.name == "wait" &&
      this.func.value.type == "id" &&
      this.func.value.value == "time"
    ) {
  ...
  var $ClassCtx = ($B.parser.$ClassCtx = function(context) {
    ...
    this.transform = function(node, rank) {
      ...
      var module_name =
        "$locals_" + $get_module(this).module.replace(/\./g, "_") + ".__name__";

      rank++;
      node.parent.insert(
        rank + 1,
        $NodeJS(
          "$" + this.name + "_" + this.random + ".__module__ = " + module_name
        )
      );
      ...
      if (this.args !== undefined) {
        ...
        kw.forEach(function(_tmp) {
          _r[rpos++] =
            '["' + _tmp.tree[0].value + '",' + _tmp.tree[1].to_js() + "]";
        });
        js[pos++] = ",[" + _r.join(",") + "]";
      } else {
        // form "class foo:"
  ...
  var $ConditionCtx = ($B.parser.$ConditionCtx = function(context, token) {
    ...
    this.to_js = function() {
      ...
      if (this.tree.length == 1) {
        ...
        // syntax "if cond : do_something" in the same line
  ...
  var $DefCtx = ($B.parser.$DefCtx = function(context) {
    ...
    this.transform = function(node, rank) {
      ...
      if (this.async) {
        enter_frame_nodes.push(
          $NodeJS("var $stack = " + "$B.frames_stack.slice()")
        );
      }
      ...
      if (this.type != "generator") {
        ...
        if (this.kwonlyargsdefaults.lengh > 0) {
          ...
          node.parent.insert(
            rank + offset++,
            $NodeJS("    __kwdefaults__ : " + "_b_.None,")
          );
        }
      }
      ...
      node.parent.insert(
        rank + offset++,
        $NodeJS(
          "    __dict__: {__class__: _b_.dict, $string_dict: {}," +
      ...
      if (this.type == "def") {
        ...
        node.parent.insert(
          rank + offset++,
          new $MarkerNode("func_end:" + CODE_MARKER)
        );
        var res = "return " + name;
        if (this.async) {
      ...
      if (this.type == "def") {
        ...
        for (
          ...
          parent.children[pos] !== $B.last(enter_frame_nodes);
          pos++
        ) {}
    ...
    this.to_js = function() {
      ...
      switch (this.real) {
        case "dict":
          ...
          return (
            "_b_.dict.$factory([" + res.join(",") + "])" + $to_js(this.tree)
          );
        case "set_comp":
          return (
            "_b_.set.$factory(" + $to_js(this.items) + ")" + $to_js(this.tree)
          );
        case "dict_comp":
          return (
            "_b_.dict.$factory(" + $to_js(this.items) + ")" + $to_js(this.tree)
          );
      }
      var packed = this.packed_indices();
      if (packed.length > 0) {
  ...
  var $EndOfPositionalCtx = ($B.parser.$EndOfConditionalCtx = function(
    context
  ) {
  ...
  var $ExceptCtx = ($B.parser.$ExceptCtx = function(context) {
    ...
    this.transform = function(node, rank) {
      ...
      if (
        last_child.context.tree &&
        last_child.context.tree[0] &&
  ...
  var $ForExpr = ($B.parser.$ForExpr = function(context) {
    ...
    this.transform = function(node, rank) {
      ...
      if ($range && scope.ntype != "generator") {
        ...
        if (!shortcut) {
          ...
          for_node.add(
            $NodeJS("else{$next" + num + " = $B.add($next" + num + ",1)}")
          );
        }
        // Add the loop body
        children.forEach(function(child) {
        ...
        if (scope.ntype == "module" && !in_loop) {
          ...
          test_range_node.add(
            $NodeJS("var $res" + num + " = $f" + num + "();")
          );

          if (this.has_break) {
            test_range_node.add(
              $NodeJS("var $no_break" + num + " = $res" + num)
            );
          }
        } else {
    ...
    this.transform_async = function(node, rank) {
      ...
      new_nodes.push(
        $NodeJS(
          "var " + type_name + " = _b_.type.$factory( " + iterable_name + ")"
        )
      );
      ...
      if (target.tree.length == 1) {
        ...
        new $RawJSCtx(
          assign,
          "await $B.promise(" + anext_name + "(" + iterable_name + "))"
        );
  ...
  var $IdCtx = ($B.parser.$IdCtx = function(context, value) {
    ...
    this.to_js = function(arg) {
      ...
      this.nonlocal =
        this.scope.nonlocals && this.scope.nonlocals[val] !== undefined;
      ...
      if (this.nonlocal || this.bound) {
        ...
        if (bscope !== undefined) {
          ...
          return (
            "$locals_" + innermost.id.replace(/\./g, "_") + '["' + val + '"]'
          );
        }
      }
          ...
          if (bound_before) {
            ...
            if (scope.binding[val]) {
              ...
              if (this_node.locals[val] === undefined) {
                ...
                if (
                  !scope.is_comp &&
                  (!scope.parent_block || !scope.parent_block.is_comp)
                ) {
      ...
      if (found.length > 0) {
        ...
        if (scope.context === undefined) {
          ...
          if (scope.ntype == "generator") {
            ...
            var scope_name =
              "$B.frames_stack[$B.frames_stack.length-1-" + up + "][1]";
            val =
              '$B.$check_def_free1("' +
              val +
              '", "' +
  ...
  var $ImportedModuleCtx = ($B.parser.$ImportedModuleCtx = function(
    context,
    name
  ) {
  ...
  var $KwArgCtx = ($B.parser.$KwArgCtx = function(context) {
    ...
    if (ctx.kwargs === undefined) {
      ...
      $_SyntaxError(context, ["keyword argument repeated"]);
    }
  ...
  var $ListOrTupleCtx = ($B.parser.$ListOrTupleCtx = function(context, real) {
    ...
    this.toString = function() {
      switch (this.real) {
        ...
        case "gen_expr":
          return (
            "(" + this.real + ") [" + this.intervals + "-" + this.tree + "]"
          );
        default:
        ...
        case "dict_or_set_comp":
          ...
          switch (this.real) {
            case "list_comp":
              ...
              js = "(function($locals_" + listcomp_name + "){" + js + "})({})";
              return js;
        ...
        if (this.tree[0].annotation) {
          ...
          if (is_not_def) {
            if (
              ...
              return (
                js +
                "$locals.__annotations__.$string_dict['" +
                this.tree[0].tree[0].value +
                "'] = " +
  ...
  var $OpCtx = ($B.parser.$OpCtx = function(context, op) {
    ...
    this.to_js = function() {
      ...
      switch (this.op) {
        ...
        case "or":
          var res =
            "$B.$test_expr($B.$test_item(" + this.tree[0].to_js() + ")||";
        ...
        case "is":
          return (
            "$B.$is(" + this.tree[0].to_js() + ", " + this.tree[1].to_js() + ")"
          );
        case "is_not":
          return this.tree[0].to_js() + "!==" + this.tree[1].to_js();
        case "+":
          return (
            "$B.add(" + this.tree[0].to_js() + ", " + this.tree[1].to_js() + ")"
          );
        case "*":
        case "-":
          var op = this.op,
  ...
  var $SubCtx = ($B.parser.$SubCtx = function(context) {
    ...
    this.to_js = function() {
      ...
      if (this.func == "getitem" && this.tree.length == 1) {
        return (
          "$B.$getitem(" + this.value.to_js() + "," + this.tree[0].to_js() + ")"
        );
      }
      var res = "",
        shortcut = false;
  ...
  var $WithCtx = ($B.parser.$WithCtx = function(context) {
    ...
    this.transform = function(node, rank) {
      ...
      if (
        ...
        if (
          this.tree[1].type != "expr" ||
          this.tree[1].tree[0].type != "list_or_tuple"
        ) {
          $_SyntaxError(context);
        }
        if (
          this.tree[0].tree[0].tree.length != this.tree[1].tree[0].tree.length
        ) {
      ...
      if (this.tree[0].alias) {
        ...
        try_node.add(
          $NodeJS("$locals" + '["' + alias + '"] = ' + this.val_name)
        );
      }
    ...
    this.transform_async = function(node, rank) {
      ...
      new_nodes.push(
        $NodeJS(
          "    " + cmtype_name + " = _b_.type.$factory(" + this.cm_name + "),"
        )
      );
      new_nodes.push(
        $NodeJS(
      ...
      catch_node.add(
        $NodeJS("var " + err_name + ' = $B.imported["_sys"].exc_info()')
      );
      var if_node = $NodeJS(
        "if(! await $B.promise(" +
          this.cmexit_name +
  ...
  var $add_line_num = ($B.parser.$add_line_num = function(node, rank) {
    if (node.type == "module") {
      ...
      if (flag) {
        // add a trailing None for interactive mode
        var js = ';$locals.$line_info = "' + line_num + "," + mod_id + '";';
      ...
      if (
        ...
        if ($B.last(node.children).context.tree[0].type != "return") {
          node.add(
            $NodeJS('$locals.$line_info = "' + line_num + "," + mod_id + '";')
          );
        }
      }
  ...
  var $previous = ($B.parser.$previous = function(context) {
    var previous =
      context.node.parent.children[context.node.parent.children.length - 2];
  ...
  var $get_docstring = ($B.parser.$get_docstring = function(node) {
    ...
    if (node.children.length > 0) {
      ...
      if (
        firstchild.context.tree &&
        firstchild.context.tree.length > 0 &&
    ...
    switch (context.type) {
      case "abstract_expr":
        ...
        switch (token) {
          ...
          case "bytes":
            return new $StringCtx(
              new $ExprCtx(context, "bytes", commas),
              value
            );
          case "int":
          ...
          case "imaginary":
            return new $ImaginaryCtx(
              new $ExprCtx(context, "imaginary", commas),
              value
            );
          case "(":
          ...
          case "[":
            return new $ListOrTupleCtx(
              new $ExprCtx(context, "list", commas),
              "list"
            );
          case "{":
      ...
      case "async":
        if (token == "def") {
          ...
          if (ntype !== "def" && ntype != "generator") {
            $_SyntaxError(context, [
              "'async " + token + "' outside async function"
            ]);
          }
          ...
          case ")":
            if (
              ...
              ["kwarg", "star_arg", "double_star_arg"].indexOf(
                $B.last(context.parent.tree).tree[0].type
              ) == -1
            ) {
      ...
      case "condition":
        if (token == ":") {
          if (
            context.tree[0].type == "abstract_expr" &&
            context.tree[0].tree.length == 0
          ) {
            // issue #965
            $_SyntaxError(context, "token " + token + " after " + context);
      ...
      case "decorator":
        if (token == "id" && context.tree.length == 0) {
          return $transition(new $DecoratorExprCtx(context), token, value);
        }
        if (token == "eol") {
          return $transition(context.parent, token);
        }
          ...
          if (context.expect == ",") {
            switch (token) {
              ...
              case ",":
                ...
                if (context.real == "dict" && context.nb_dict_items() % 2) {
                  $_SyntaxError(
                    context,
      ...
      case "expr":
        ...
        switch (token) {
          ...
          case "augm_assign":
            ...
            if (context.expect == ",") {
              return new $AbstractExprCtx(
                new $AugmentedAssignCtx(context, value),
                true
              );
            }
            return $transition(context.parent, token, value);
            ...
            if (
              ...
              if (context.tree.length == 1) {
                ...
                if (["id", "sub", "attribute"].indexOf(child.type) > -1) {
                  ...
                  return new $AbstractExprCtx(
                    new $AnnotationCtx(child.tree[0]),
                    false
                  );
                }
              }
          ...
          case "eol":
            if (
              ["dict_or_set", "list_or_tuple"].indexOf(context.parent.type) ==
              -1
            ) {
      ...
      case "expr_not":
        if (token == "in") {
          // expr not in : operator
          context.parent.tree.pop();
          return new $AbstractExprCtx(
            new $OpCtx(context.parent, "not_in"),
            false
          );
        }
        $_SyntaxError(context, "token " + token + " after " + context);
      ...
      case "for":
        switch (token) {
          case "in":
            return new $AbstractExprCtx(
              new $ExprCtx(context, "target list", true),
              false
            );
          case ":":
            if (
              context.tree.length < 2 || // issue 638
      ...
      case "from":
        switch (token) {
          case "id":
            ...
            if (context.expect == "alias") {
              context.aliases[context.names[context.names.length - 1]] = value;
              context.expect = ",";
              return context;
            }
          case ".":
          ...
          case "op":
            ...
            if (op == "*") {
              ...
              if (context.has_end_positional) {
                $_SyntaxError(context, ["duplicate / in function parameters"]);
              } else if (context.has_star_arg) {
                $_SyntaxError(context, ["/ after * in function parameters"]);
              }
        ...
        switch (token) {
          case "id":
            ...
            if (context.expect == "qual") {
              context.expect = ",";
              context.tree[context.tree.length - 1].name += "." + value;
              context.tree[context.tree.length - 1].alias += "." + value;
              return context;
            }
            if (context.expect == "alias") {
              context.expect = ",";
      ...
      case "node":
        switch (token) {
          ...
          case "while":
            return new $AbstractExprCtx(
              new $ConditionCtx(context, token),
              false
            );
          case "elif":
            var previous = $previous(context);
            if (
              ["condition"].indexOf(previous.type) == -1 ||
          ...
          case "finally":
            var previous = $previous(context);
            if (
              ["try", "except"].indexOf(previous.type) == -1 &&
      ...
      case "raise":
        switch (token) {
          case "id":
            if (context.tree.length == 0) {
              return new $IdCtx(new $ExprCtx(context, "exc", false), value);
            }
            break;
          case "from":
            if (context.tree.length > 0) {
      ...
      case "target_list":
        switch (token) {
          case "id":
            if (context.expect == "id") {
              context.expect = ",";
              return new $IdCtx(new $ExprCtx(context, "target", false), value);
            }
          case "op":
            ...
            if (raw) {
              ...
              if (src.charAt(end + 1) == "\n") {
                ...
                if (search === null) {
                  $_SyntaxError(
                    context,
                    "(unicode error) " + "malformed \\N character escape",
                    pos
                  );
                }
                ...
                if ($B.unicodedb !== undefined) {
                  ...
                  if (search === null) {
                    $_SyntaxError(
                      context,
                      "(unicode error) " + "unknown Unicode character name",
                      pos
                    );
                  }
      ...
      switch (car) {
        ...
        case "9":
          ...
          if (res) {
            ...
            if (res) {
              ...
              $pos = pos;
              if (res[2] !== undefined) {
                context = $transition(context, "imaginary", rmu(res[1]));
              } else {
                context = $transition(context, "int", [10, rmu(res[0])]);
              }
            }
        ...
        case "!":
          ...
          for (var op_sign in $operators) {
            if (
              op_sign == src.substr(pos, op_sign.length) &&
  ...
  var $create_root_node = ($B.parser.$create_root_node = function(
    src,
    module,
    locals_id,
    parent_block,
  ...
  var brython = ($B.parser.brython = function(options) {
    ...
    if ($B.isWebWorker || $B.isNode) {
      $href_elts.pop();
    } // WebWorker script is in the web_workers subdirectory
    ...
    if (options.python_paths) {
      options.python_paths.forEach(function(path) {
        ...
        $B.path.push(path);
        if (
          path.slice(-7).toLowerCase() == ".vfs.js" &&
          (prefetch === undefined || prefetch === true)
        ) {
          $B.path_importer_cache[path + "/"] = $B.imported[
            "_importlib"
          ].VFSPathFinder(path);
        }
        if (lang) {
    ...
    try {
      var root = $B.py2js(src, name, name),
        ...
        script = {
          ...
          $src: src,
          __file__:
            $B.script_path + ($B.script_path.endsWith("/") ? "" : "/") + name
        };
