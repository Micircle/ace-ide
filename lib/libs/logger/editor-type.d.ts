...
export interface FoldLine {
    ...
    getNextFoldTo(row: number, column: number): null | {
        fold: Fold;
        kind: string;
    };
...
interface Folding {
    ...
    unfold(location: null | number | Point | Range, expandInner?: boolean): Fold[] | undefined;
...
export interface EditorOptions extends EditSessionOptions, MouseHandlerOptions, VirtualRendererOptions {
...
export declare type CommandLike = Command | ((editor: Editor) => void);
...
export declare type MarkerRenderer = (html: string[], range: Range, left: number, top: number, config: any) => void;
...
export interface SyntaxMode {
    ...
    transformAction(state: string, action: string, editor: Editor, session: EditSession, text: string): any;
...
export interface Config {
    ...
    loadModule(moduleName: string | [string, string], onLoad: (module: any) => void): void;
...
export interface EditSession extends EventEmitter, OptionsProvider, Folding {
    selection: Selection;
    _emit(name: string, data?: any): Function;
    on(name: 'changeFold', callback: (obj: {
        data: Fold;
        action: string;
    }) => void): Function;
    ...
    on(name: 'tokenizerUpdate', callback: (obj: {
        data: {
            first: number;
            last: number;
        };
    ...
    addMarker(range: Range, className: string, type: "fullLine" | "screenLine" | "text" | MarkerRenderer, inFront?: boolean): number;
...
declare type execEventHandler = (obj: {
    editor: Editor;
    command: Command;
    args: any[];
}) => void;
export interface CommandManager extends EventEmitter {
    ...
    bindKey(key: string | {
        mac?: string;
        win?: string;
    }, command: CommandLike, position?: number): void;
}
...
export interface Editor extends OptionsProvider, EventEmitter {
    ...
    on(name: 'changeSelectionStyle', callback: (obj: {
        data: string;
    }) => void): Function;
    on(name: 'changeSession', callback: (obj: {
        session: EditSession;
        oldSession: EditSession;
    }) => void): Function;
    on(name: 'copy', callback: (obj: {
        text: string;
    }) => void): Function;
    on(name: 'focus', callback: (e: Event) => void): Function;
    on(name: 'paste', callback: (obj: {
        text: string;
    }) => void): Function;
...
declare type CompleterCallback = (error: any, completions: Completion[]) => void;
interface Completer {
